...
class NovArchive:
    ...
    def __init__(self, lbd, all_std, k=15):
        self.all_bd=lbd
    ...
    def update(self,new_bd,new_std):
        oldsize=len(self.all_bd)
        self.all_bd=self.all_bd + new_bd
        self.all_std=self.all_std + new_std
        self.kdtree=KDTree(self.all_bd)
    ...
    def get_nov(self,bd, population=[]):
        ...
        d=dpop + [(darch[i],self.all_std[ind[i]]) for i in range(len(darch))] #nearest neigbour
        d.sort(key = lambda e:e[0])
        d,std = list(map(list,zip(*d)))
        loc_fit = len([i for i in std[1:self.k +1] if i<std[0]])
        if (d[0]!=0):
        ...
        nov = sum(d[:self.k+1])/self.k
        return nov, loc_fit# as the indiv is in the population, the first value is necessarily a 0.
...
def updateNovelty(population, offspring, archive, k=15, add_strategy="random", _lambda=6, verbose=False):
   ...
   if (archive) and (archive.size()>=k):
       ...
       for ind in population:
           ind.novelty, ind.loc_fit=archive.get_nov(ind.bd, population)
   ...
   if(add_strategy=="random"):
       ...
       std=[np.std(offspring[l[i]]) for i in range(_lambda)]
       ...
       std=[np.std(soff[i]) for i in range(ilast,len(soff))]
       if (verbose):
   ...
   if(archive==None):
       archive=NovArchive(lbd, std, k)
   else:
       archive.update(lbd,std)
