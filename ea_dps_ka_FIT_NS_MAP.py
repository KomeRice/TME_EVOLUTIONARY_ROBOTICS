...
import grid_management_MAP as grid_management
...
def eval_ka(angles, lengths, target_pos, walls, resdir=None, render=False, dump=False, name=""):
    a=Arm(lengths, walls)
    v, allPoints, inters = a.fw_kinematics(angles)
...
def launch_ea(mu=100, lambda_=200, cxpb=0.3, mutpb=0.7, ngen=100, verbose=False, resdir="res"):
    ...
    for ind in population:
        ...
        curFit = ind.log[registered_envs[env_name]['watch_max']]
        if curFit > max_fit:
            best_ind = ind
            max_fit = curFit
    ...
    history = [best_ind]
    best_fit = max_fit
    best_fit_dict = {0: best_fit}
    print(f'Gen0: Best ind fit = {max_fit}')
    ...
    for gen in range(1, ngen + 1):
        ...
        if max_fit > best_fit:
            print(f'Better fitness attained @ Gen{gen}: old_best = {best_fit} new_best = {max_fit}')
            history.append(best_ind)
            best_fit = max_fit
            best_fit_dict[gen] = max_fit
    ...
    return population, None, paretofront, grid, history, best_fit_dict
...
if (__name__ == "__main__"):
    ...
    pop, logbook, paretofront, grid, hist, bsdict = launch_ea(mu=mu, lambda_=lambda_, ngen=ngen, resdir=resdir)


    cdir="completed_runs_MAP"
    try:
        os.mkdir(cdir)
    ...
    with open(f'{cdir}/{resdir}/hist.csv', 'w+') as f:
        f.write('gen;fit;param;v;inter\n')
        for ind, i in zip(hist, bsdict):
            f.write(f'{i};{bsdict[i]};{[pa for pa in ind]};{ind.log["allPoints"]}\n')

    with open(f'{cdir}/{resdir}/grid.txt', 'w+') as f:
        f.write(str(len(grid) / (100 * 100)))
