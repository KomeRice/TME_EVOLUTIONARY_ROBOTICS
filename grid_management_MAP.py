...
def get_grid_coord(bd, dim=[100, 100], min_v=[0,0], max_v=[1, 1]):
    ...
    stepX = abs(max_v[0]-min_v[0])/dim[0]
    stepY = abs(max_v[1]-min_v[1])/dim[1]
    x = math.floor(bd[0] / stepX) + 50
    y = math.floor(bd[1] / stepY) + 50
    if x < 0:
        x = 0
    if x > 99:
        x = 99
...
def add_to_grid(grid,ind, local_quality, dim=[100, 100], min_v=[0,0], max_v=[1, 1]):
    ...
    pos = get_grid_coord(ind.bd,dim,min_v,max_v)
    if not pos in grid:
        grid[pos] = ind
    else:
        if grid[pos].fit < local_quality:
            grid[pos] =ind
...
def selGrid(grid,k):
    chosen = []
    for i in range(k):
        chosen.append(random.choice(list(grid)))
    return chosen